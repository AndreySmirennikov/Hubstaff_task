...
class GmailHelper {
    ...
    async waitForHubstaffEmail(recipient: string, timeout = 180000): Promise<string | null> {
        ...
        while (Date.now() - start < timeout) {
            ...
            if (matchingEmails.length > 0) {
                ...
                const email = matchingEmails.sort((a, b) => (b.date?.getTime() || 0) - (a.date?.getTime() || 0))[0];
                ...
                if (link) {
                    ...
                    console.error('[GMAIL-HELPER] ERROR: Could not extract confirmation link from the email content.');
                }
            }
        ...
        console.error(`[GMAIL-HELPER] TIMEOUT: Did not find a Hubstaff confirmation email for ${recipient} within ${timeout / 1000} seconds.`);
        return null;
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.imap.openBox('INBOX', true, (err, box) => {
                ...
                this.imap.search(['ALL'], (err, results) => {
                    ...
                    f.once('end', () => {
                        Promise.all(parsingPromises).then(emails => {
    ...
    private extractConfirmationLink(rawEmail: string): string | null {
        const pattern = /href=3D"((https:\/\/account\.hubstaff\.com\/confirm_account\/[a-zA-Z0-9\-_=\s]+))"/;
